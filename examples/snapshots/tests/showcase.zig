...
test "default snapshot behavior - colors with masking" {
    ...
    try testing.expectSnapshot(result.stdout, @src(), "colors_default", .{});
}
...
test "plain text snapshot - strip colors and no masking" {
    ...
    try testing.expectSnapshot(result.stdout, @src(), "colors_plain", .{ .ansi = false, .mask = false });
}
...
test "colors preserved but no masking" {
    ...
    try testing.expectSnapshot(result.stdout, @src(), "table_with_colors", .{ .mask = false });
}
...
test "dynamic content masking" {
    ...
    try testing.expectSnapshot(dynamic_content, @src(), "dynamic_masked", .{});
}
...
test "json output snapshot" {
    ...
    try testing.expectSnapshot(result.stdout, @src(), "json_output", .{ .ansi = false, .mask = false });
}
...
test "log output with timestamp masking" {
    ...
    try testing.expectSnapshot(result.stdout, @src(), "logs_masked", .{});
}
...
test "help text snapshot" {
    ...
    try testing.expectSnapshot(result.stderr, @src(), "help_output", .{ .ansi = false, .mask = false });
}
...
test "error output with colors" {
    ...
    try testing.expectSnapshot(result.stderr, @src(), "error_with_colors", .{});
}
...
test "utility functions showcase" {
    ...
    try testing.expectSnapshot(masked, @src(), "utility_masked", .{ .ansi = false, .mask = false });
    ...
    try testing.expectSnapshot(stripped, @src(), "utility_stripped", .{ .ansi = false, .mask = false });
}
...
test "complex mixed content showcase" {
    ...
    try testing.expectSnapshot(output.items, @src(), "mixed_full", .{}); // Default: mask + ansi
    try testing.expectSnapshot(output.items, @src(), "mixed_colors_only", .{ .mask = false }); // ANSI but no masking
    try testing.expectSnapshot(output.items, @src(), "mixed_masked_plain", .{ .ansi = false }); // Masked but no ANSI
}

// Test 12: Performance and edge cases
test "edge cases and special content" {
    // Empty content
    try testing.expectSnapshot("", @src(), "empty_content", .{});
