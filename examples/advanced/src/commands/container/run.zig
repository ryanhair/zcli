...
pub const meta = .{
    ...
    .options = .{
        .detach = .{ .description = "Run container in background", .short = 'd' },
        .interactive = .{ .description = "Keep STDIN open", .short = 'i' },
        .tty = .{ .description = "Allocate a pseudo-TTY", .short = 't' },
        .name = .{ .description = "Assign a name to the container" },
        .env = .{ .description = "Set environment variables (can be used multiple times)", .short = 'e' },
        .publish = .{ .description = "Publish container ports to host (can be used multiple times)", .short = 'p' },
        .volume = .{ .description = "Bind mount a volume (can be used multiple times)", .short = 'v' },
        .workdir = .{ .description = "Working directory inside the container", .short = 'w' },
        .rm = .{ .description = "Automatically remove the container when it exits" },
        .restart = .{ .description = "Restart policy (no, on-failure, always, unless-stopped)" },
        .memory = .{ .description = "Memory limit (e.g., 512m, 2g)", .short = 'm' },
        .cpus = .{ .description = "Number of CPUs" },
    },
};
