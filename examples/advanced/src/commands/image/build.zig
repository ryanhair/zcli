...
pub const meta = .{
    ...
    .options = .{
        .tag = .{ .description = "Name and optionally tag in 'name:tag' format", .short = 't' },
        .file = .{ .description = "Name of the Dockerfile (default: 'PATH/Dockerfile')", .short = 'f' },
        .build_arg = .{ .description = "Set build-time variables (can be used multiple times)" },
        .target = .{ .description = "Set the target build stage to build" },
        .no_cache = .{ .description = "Do not use cache when building the image" },
        .pull = .{ .description = "Always attempt to pull a newer version of the image" },
        .quiet = .{ .description = "Suppress the build output and print image ID on success", .short = 'q' },
        .rm = .{ .description = "Remove intermediate containers after a successful build (default true)" },
        .force_rm = .{ .description = "Always remove intermediate containers" },
        .squash = .{ .description = "Squash newly built layers into a single new layer" },
        .platform = .{ .description = "Set platform if server is multi-platform capable" },
        .progress = .{ .description = "Set type of progress output (auto, plain, tty)" },
    },
};
