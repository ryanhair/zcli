...
pub fn build(b: *std.Build) void {
    ...
    const test_projects = [_]ProjectInfo{
        ...
        .{ .name = "zcli-help-plugin", .path = "packages/plugins/zcli-help" },
        .{ .name = "zcli-not-found-plugin", .path = "packages/plugins/zcli-not-found" },
    };
    ...
    for (test_projects) |project| {
        ...
        const project_test_step = b.step(
            b.fmt("test-{s}", .{project.name}), 
            b.fmt("Run tests for {s}", .{project.name})
        );
    ...
    for (example_projects) |example| {
        ...
        const example_build_step = b.step(
