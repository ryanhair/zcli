...
- [x] 21. **Global options half-implemented** — short options "assume boolean for now" (`registry.zig:916-917`); `convertValue` supports only bool/u16/u32/[]const u8 while `plugin_types.option()` accepts more (`registry.zig:951-960`). Support the full type set or reject unsupported types at declaration.
...
- [x] 27. **Context triplication** *(one generic `ContextFor(plugins)` in new context.zig; `zcli.Context` = plugin-less instantiation, `TestContext` = alias, registry re-exports `ContextFor(new_plugins)` — Zig memoization makes equal plugin lists the same type)* — the Context interface is defined three times (`registry.zig:278-396`, `zcli.zig:107-203`, `zcli.zig:264-341`); `getCommandDescription` copy-pasted verbatim in all three. Single source of truth.
- [x] 28. **Split `add/command.zig`** *(three files: `_wizard.zig` interactive UI, `_generate.zig` source rendering + file placement, `command.zig` ~100-line dispatch; declarative JSON mode had already been superseded by scaffold/splice. Enabler: command discovery now skips underscore-prefixed files/dirs — the helper-file convention — and parsePath refuses to scaffold `_`-names)* — wizard prompt IO, declarative scaffold, source rendering, and ANSI paint helpers in one file. (Split plan already sketched: generate/declarative/wizard/command.)
- [x] 29. **Legacy `build_utils.zig` cleanup** *(slimmed to a 24-line module root; real codegen tests moved to code_generation.zig and collected via a test{} wiring block — mutation-verified; main.zig's dormant stale-signature duplicates and the fictional transformer-pipeline tests deleted)* — 772-line doc-header + stale test grab-bag incl. a no-op `expect(true)` test (`:25-36`). Also `build_utils/main.zig` contains never-run tests asserting a generator signature that no longer exists (`:465`, `:547` vs `code_generation.zig:180-190`) — fix and wire them, or delete.
