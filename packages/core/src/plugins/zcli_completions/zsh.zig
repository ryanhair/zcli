...
pub fn generate(
    ...
    // Dynamic-completion callback helper (ADR-0026), emitted only when some
    // command declares a `.complete` function hook.
    ...
    // to a sub-context: the dynamic helper (called from a positional action) needs
    // the whole line and the real cursor index to call `__complete`.
    if (tree.hasDynamicArg(root)) {
...
fn appendPositionalSpecs(
    ...
    for (args, 1..) |arg, idx| {
        ...
        // A function hook takes precedence: the action calls the dynamic helper,
        // which runs `__complete` and offers the live candidates.
        const is_hook = if (arg.complete) |c| c == .hook else false;

        const spec = if (is_hook) blk: {
            const esc_msg = try escape.zsh(arena, msg);
            break :blk try std.fmt.allocPrint(arena, "{s}:{s}:_{s}_zcli_complete", .{ pos, esc_msg, app_name });
