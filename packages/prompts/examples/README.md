...
- **Searchable lists.** Set `.search = true` on `select` or `multiSelect`.
  ...
  toggles while Enter selects or commits. ASCII Space is not query text, and a
  multi-select keeps selections that filtering hides. Plain `select` also
  accepts Space like Enter.
