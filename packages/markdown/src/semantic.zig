//! Semantic color support for markdown
//!
//! Types are imported from theme (the canonical source for semantic colors).
//! This module provides markdown-specific parsing and multi-version compilation.

const std = @import("std");
const theme = @import("theme");

// Re-export canonical types from theme
pub const TerminalCapability = theme.TerminalCapability;
pub const SemanticRole = theme.SemanticRole;
...
pub fn parseWithSemantics(comptime markdown: []const u8, comptime palette: Palette, comptime capability: TerminalCapability) []const u8 {
    comptime {
...
pub fn parse(comptime markdown: []const u8, comptime palette: Palette) ComptimeHelp {
    return .{
...
test "parse with semantics - 16 color" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, result, "38;2;") == null); // Should NOT be RGB
}
...
test "multi-version parse" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, help.true_color, "38;2;") != null);
...
test "parse markdown inside semantic tags" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, result, "38;2;") != null); // RGB color
