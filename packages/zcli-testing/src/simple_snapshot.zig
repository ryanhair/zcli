...
/// Simple runtime-only snapshot testing - no build-time complexity
pub fn expectSnapshot(
    ...
    const update_mode = std.process.getEnvVarOwned(std.heap.page_allocator, "UPDATE_SNAPSHOTS") catch null;
    defer if (update_mode) |mode| std.heap.page_allocator.free(mode);

    if (update_mode != null) {
        // Update mode - create/update the snapshot
        try updateSnapshot(std.heap.page_allocator, snapshot_dir, snapshot_name, actual);
        std.debug.print("✅ Updated snapshot: {s}\n", .{snapshot_file});
        return;
    }

    // Try to read existing snapshot
    const expected = std.fs.cwd().readFileAlloc(std.heap.page_allocator, snapshot_file, 1024 * 1024) catch |err| switch (err) {
        error.FileNotFound => {
            ...
            std.debug.print("Run with -Dupdate-snapshots=true to create this snapshot.\n\n", .{});
    ...
    if (!std.mem.eql(u8, expected, actual)) {
        ...
        std.debug.print("Run with -Dupdate-snapshots=true to update this snapshot.\n\n", .{});
...
/// Snapshot testing with automatic masking of dynamic content
pub fn expectSnapshotWithMasking(
    actual: []const u8,
    comptime location: std.builtin.SourceLocation,
    comptime snapshot_name: []const u8,
) !void {
    const allocator = std.heap.page_allocator;
    const masked_actual = try maskDynamicContent(allocator, actual);
    defer allocator.free(masked_actual);

    return expectSnapshot(masked_actual, location, snapshot_name);
}
...
/// Snapshot testing that preserves ANSI escape sequences
/// Use this when testing colored CLI output
pub fn expectSnapshotAnsi(
    actual: []const u8,
    comptime location: std.builtin.SourceLocation,
    comptime snapshot_name: []const u8,
) !void {
    // For ANSI snapshots, we store the exact output including escape codes
    // but provide better diff output that shows both clean text and ANSI differences
    return expectSnapshotWithAnsiDiff(actual, location, snapshot_name);
}

fn expectSnapshotWithAnsiDiff(
    actual: []const u8,
    comptime location: std.builtin.SourceLocation,
    comptime snapshot_name: []const u8,
) !void {
    ...
    if (update_mode != null) {
        // Update mode - create/update the snapshot (preserving ANSI)
        try updateSnapshot(std.heap.page_allocator, snapshot_dir, snapshot_name, actual);
        std.debug.print("✅ Updated ANSI snapshot: {s}\n", .{snapshot_file});
        return;
    }
    ...
    const expected = std.fs.cwd().readFileAlloc(std.heap.page_allocator, snapshot_file, 1024 * 1024) catch |err| switch (err) {
        error.FileNotFound => {
            // No snapshot exists - this is a new test
            std.debug.print("\n❌ MISSING ANSI SNAPSHOT\n", .{});
            std.debug.print("Test: {s}:{d}\n", .{ location.file, location.line });
            std.debug.print("Snapshot: {s}\n", .{snapshot_file});
            std.debug.print("Actual output (with ANSI):\n{s}\n", .{actual});
            std.debug.print("Run with -Dupdate-snapshots=true to create this snapshot.\n\n", .{});
            return error.SnapshotMissing;
        },
        else => return err,
    ...
    // Compare ANSI-preserved output directly (exact match including escape codes)
    if (!std.mem.eql(u8, expected, actual)) {
        ...
        std.debug.print("\n--- Expected (with ANSI) ---\n{s}\n", .{expected});
        std.debug.print("--- Actual (with ANSI) ---\n{s}\n", .{actual});

        // Show ANSI-aware diff
        printAnsiAwareDiff(expected, actual);
...
fn printAnsiAwareDiff(expected: []const u8, actual: []const u8) void {
    std.debug.print("\n--- ANSI-Aware Diff ---\n", .{});

    // Strip ANSI codes for comparison but show original in output
    const allocator = std.heap.page_allocator;
    const expected_clean = stripAnsi(allocator, expected) catch expected;
    defer if (expected_clean.ptr != expected.ptr) allocator.free(expected_clean);
    const actual_clean = stripAnsi(allocator, actual) catch actual;
    defer if (actual_clean.ptr != actual.ptr) allocator.free(actual_clean);

    // Show differences in both the clean text and the ANSI codes
    var exp_lines = std.mem.splitScalar(u8, expected, '\n');
    var act_lines = std.mem.splitScalar(u8, actual, '\n');
    var exp_clean_lines = std.mem.splitScalar(u8, expected_clean, '\n');
    var act_clean_lines = std.mem.splitScalar(u8, actual_clean, '\n');
    ...
    while (true) {
        const exp_line = exp_lines.next();
        const act_line = act_lines.next();
        const exp_clean_line = exp_clean_lines.next();
        const act_clean_line = act_clean_lines.next();

        if (exp_line == null and act_line == null) break;

        const exp_text = exp_line orelse "";
        const act_text = act_line orelse "";
        const exp_clean_text = exp_clean_line orelse "";
        const act_clean_text = act_clean_line orelse "";

        if (exp_line == null) {
            std.debug.print("+ {d:3}   {s} (clean: {s})\n", .{ line_num, act_text, act_clean_text });
        } else if (act_line == null) {
            std.debug.print("- {d:3}   {s} (clean: {s})\n", .{ line_num, exp_text, exp_clean_text });
        } else {
            // Show both clean comparison and ANSI-aware difference
            if (!std.mem.eql(u8, exp_clean_text, act_clean_text)) {
                std.debug.print("- {d:3}   {s} (clean: {s})\n", .{ line_num, exp_text, exp_clean_text });
                std.debug.print("+ {d:3}   {s} (clean: {s})\n", .{ line_num, act_text, act_clean_text });
            } else {
                // Same clean text, different ANSI codes
                std.debug.print("~ {d:3}   ANSI codes differ\n", .{line_num});
