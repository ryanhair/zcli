...
test "full integration: theme creation to rendering" {
    ...
    const complex_string = theme.theme("Critical Error!")
        .brightRed()
        .onWhite()
        .bold()
        .underline();
...
test "cross-capability rendering consistency" {
    ...
    const styled_text = theme.theme("Hello World").green().bold();
...
test "memory safety with different content types" {
    ...
    const theme_ctx = theme.Theme.init(&(std.process.Environ.Map.init(std.testing.allocator)), std.testing.io);
    var aw: std.Io.Writer.Allocating = .init(testing.allocator);
    defer aw.deinit();

    // Test with integer
    const int_themed = theme.theme(@as(i32, -42)).red();
    ...
    // Test with float
    const float_themed = theme.theme(@as(f32, 3.14)).blue();
    ...
    // Test with boolean
    const bool_themed = theme.theme(true).green();
...
test "compile-time vs runtime equivalence" {
    ...
    const styled = comptime theme.theme("CompTime Test").red().bold();
    const theme_ctx = theme.Theme{
        .capability = .ansi_16,
        .is_tty = true,
        .color_enabled = true,
...
test "error handling and edge cases" {
    ...
    const empty_themed = theme.theme("");
    const theme_ctx = theme.Theme.init(&(std.process.Environ.Map.init(std.testing.allocator)), std.testing.io);
    ...
    const original = theme.theme("string").red();
    ...
    const heavily_styled = theme.theme("text").brightMagenta().onCyan().bold().italic().underline();
...
test "platform detection integration" {
    ...
    for (valid_caps) |cap| {
        const theme_ctx = theme.Theme.initWithCapability(cap, std.testing.io);
