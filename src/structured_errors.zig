...
pub const StructuredError = union(enum) {
    ...
    pub fn description(self: @This(), allocator: std.mem.Allocator) ![]u8 {
        return switch (self) {
            ...
            .argument_too_many => |ctx| std.fmt.allocPrint(
                allocator,
                "Too many arguments provided. Expected {d} arguments",
                .{ctx.position}
            ),
            
            .option_unknown => |ctx| std.fmt.allocPrint(
                allocator,
                "Unknown option '{s}{s}'",
                .{ if (ctx.is_short) "-" else "--", ctx.option_name }
            ),
            .option_missing_value => |ctx| std.fmt.allocPrint(
                allocator,
                "Option '{s}{s}' requires a value of type: {s}",
                .{ if (ctx.is_short) "-" else "--", ctx.option_name, ctx.expected_type.? }
            ),
            .option_invalid_value => |ctx| std.fmt.allocPrint(
                allocator,
                "Invalid value '{s}' for option '{s}{s}'. Expected type: {s}",
                .{ ctx.provided_value.?, if (ctx.is_short) "-" else "--", ctx.option_name, ctx.expected_type.? }
            ),
            .option_boolean_with_value => |ctx| std.fmt.allocPrint(
                allocator,
                "Boolean option '{s}{s}' does not accept a value (got '{s}')",
                .{ if (ctx.is_short) "-" else "--", ctx.option_name, ctx.provided_value.? }
            ),
            
            .command_not_found => |ctx| std.fmt.allocPrint(
                allocator,
                "Unknown command '{s}'",
                .{ctx.command_name}
            ),
            .subcommand_not_found => |ctx| std.fmt.allocPrint(
                allocator,
                "Unknown subcommand '{s}' for command path: {s}",
                .{ ctx.command_name, if (ctx.command_path.len > 0) ctx.command_path[ctx.command_path.len - 1] else "root" }
            ),
            
            .build_command_discovery_failed => |ctx| std.fmt.allocPrint(
                allocator,
                "Command discovery failed in '{s}': {s}",
                .{ ctx.file_path.?, ctx.details }
            ),
            .build_registry_generation_failed => |ctx| std.fmt.allocPrint(
                allocator,
                "Registry generation failed: {s}",
                .{ctx.details}
            ),
...
pub const ErrorBuilder = struct {
    ...
    pub fn missingRequiredArgument(field_name: []const u8, position: usize, expected_type: []const u8) StructuredError {
        return StructuredError{ 
            .argument_missing_required = ArgumentErrorContext.missingRequired(field_name, position, expected_type) 
        };
    }
    ...
    pub fn invalidArgumentValue(field_name: []const u8, position: usize, provided_value: []const u8, expected_type: []const u8) StructuredError {
        return StructuredError{
            .argument_invalid_value = ArgumentErrorContext.invalidValue(field_name, position, provided_value, expected_type)
        };
    }
    ...
    pub fn unknownOption(option_name: []const u8, is_short: bool) StructuredError {
        return StructuredError{
            .option_unknown = OptionErrorContext.unknown(option_name, is_short)
        };
    }
    ...
    pub fn missingOptionValue(option_name: []const u8, is_short: bool, expected_type: []const u8) StructuredError {
        return StructuredError{
            .option_missing_value = OptionErrorContext.missingValue(option_name, is_short, expected_type)
        };
    }
    ...
    pub fn invalidOptionValue(option_name: []const u8, is_short: bool, provided_value: []const u8, expected_type: []const u8) StructuredError {
        return StructuredError{
            .option_invalid_value = OptionErrorContext.invalidValue(option_name, is_short, provided_value, expected_type)
        };
    }
    ...
    pub fn unknownCommand(command_name: []const u8, command_path: []const []const u8) StructuredError {
        return StructuredError{
            .command_not_found = CommandErrorContext.unknown(command_name, command_path)
        };
    }
...
test "structured error context information" {
    ...
    const error1 = StructuredError{
        .option_invalid_value = OptionErrorContext.invalidValue("count", false, "abc", "integer")
    };
