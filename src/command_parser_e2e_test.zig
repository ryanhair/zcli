...
test "e2e: options after arguments" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"input.txt", "--verbose", "output.txt", "--count", "10"}
    );
    defer result.deinit();
...
test "e2e: options before arguments" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"--verbose", "--count", "5", "input.txt", "output.txt"}
    );
    defer result.deinit();
...
test "e2e: fully interleaved options and arguments" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"--debug", "input.txt", "--count", "3", "--verbose", "output.txt"}
    );
    defer result.deinit();
...
test "e2e: array options mixed with other options" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"--files", "first.txt", "--verbose", "--files", "second.txt", "--numbers", "42"}
    );
    defer result.deinit();
...
test "e2e: array options with arguments mixed" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"build", "--files", "main.zig", "release", "--files", "utils.zig", "--verbose"}
    );
    defer result.deinit();
...
test "e2e: git-like command" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"my-repo", "--bare", "--template", "/path/to/template"}
    );
    defer result.deinit();
...
test "e2e: docker-like command with filters" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"--filter", "status=running", "--all", "--filter", "name=web", "--quiet"}
    );
    defer result.deinit();
...
test "e2e: build tool with complex options" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        &.{
            ...
            "--features", "ssl", "--features", "json",
            "--define", "VERSION=1.0", "--define", "DEBUG=false",
            "--verbose"
        }
    );
    defer result.deinit();
...
test "e2e: double dash separator" {
    ...
    const result = try command_parser.parseCommandLine(
        ...
        null,
        allocator,
        &.{"grep", "pattern", "--verbose", "--", "--looks-like-option", "file.txt"}
    );
    defer result.deinit();
...
test "e2e: missing required arguments" {
    ...
    const result = command_parser.parseCommandLine(
        BasicArgs,
        BasicOptions,
        null,
        allocator,
        &.{"--verbose"} // Only options, no required args
    );
...
test "e2e: advanced example container ls scenarios" {
    ...
    // 1. Multiple filters with other options
    {
        const result = try command_parser.parseCommandLine(
            ...
            null,
            allocator,
            &.{"--filter", "status=running", "--filter", "name=web", "--all"}
        );
        defer result.deinit();
