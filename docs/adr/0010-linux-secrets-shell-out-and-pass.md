...
- **The helper is chosen by us, not by the environment.** "Present at run time" means *installed in one of a fixed list of absolute directories* (`/usr/bin`, `/bin`, `/usr/local/bin`, the NixOS system profile, Linux Homebrew, then `~/.nix-profile/bin` and `~/.local/bin`), not "first hit on `PATH`". `std.process.spawn` resolves a bare `argv[0]` against the *parent* environment's `PATH`, so spawning the names `pass`/`secret-tool` would let any earlier `PATH` entry be handed a decrypted credential on stdin; the backends resolve and pin an absolute path instead, and the availability probes use that same lookup so "is it present" and "which one runs" are one question. There is no environment-variable override — it would reintroduce exactly the environment-controlled selection being removed — so a helper installed somewhere exotic must be symlinked into a listed directory. This does not extend inside `pass`, which is a shell script and resolves its own `gpg`/`tree`/interpreter through `PATH`; that is inherent to shelling out to a script, and it is also why the ambient environment below is forwarded whole rather than trimmed to an allowlist (a trimmed environment would have to carry `PATH` regardless).
