...
- **Linux** → the freedesktop Secret Service. *(Superseded by [ADR-0010](0010-linux-secrets-shell-out-and-pass.md): the backend now shells out to `secret-tool` instead of linking libsecret — which unbreaks musl — and adds `pass` as a runtime-detected second backend for headless environments. The original libsecret-linking description is retained below for history.)* Originally via **libsecret** (`secret_service_linux.zig`), using the `secret_password_{store,lookup,clear}_sync` helpers keyed by the same `service`/`account` attributes. libsecret's password API is NUL-terminated, so values are base64-encoded to preserve the opaque-bytes contract (documented; a value inspected via `secret-tool` shows base64).
